...
const Enquiries = () => {
  ...
  const markAsRead = async (id) => {
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiries/${id}`, {
        method: 'PUT',
  ...
  const archiveMessage = async (id) => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_BASE_URL}/api/enquiries/${id}`, { method: 'DELETE' });
      if (res.ok) fetchEnquiries();
    } catch (err) {
      console.error(err);
    }
